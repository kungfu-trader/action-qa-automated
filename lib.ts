...
const getFileList = (argv: Argv) => {
    const artifactMap = argv.artifactName ? [argv.artifactName] : getArtifactMap();
...
const scanFolder = ({ bucketPrebuilt, artifact, version }: ScanFolder): Array<any> => {
...
function awsCall(args: Array<string>, opts: { [key: string]: string | boolean }) {
